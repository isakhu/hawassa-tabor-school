...
async def seed_admin():
    ...
    async with AsyncSessionLocal() as session:
        ...
        if existing:
            changed = False
            if existing.full_name != settings.ADMIN_FULL_NAME: existing.full_name = settings.ADMIN_FULL_NAME; changed = True
            if not verify_password(settings.ADMIN_PASSWORD, existing.password_hash): existing.password_hash = hash_password(settings.ADMIN_PASSWORD); changed = True
...
for router in (auth_router, students_router, teachers_router, classes_router, attendance_router, grades_router, academic_router, grade_workflow_router, teacher_dashboard_router, class_head_dashboard_router, student_portal_router, class_head_attendance_router, grade_submission_router):
