...
function roleBadgeGradient(role: ROLES) {
  switch (role) {
    case ROLES.ADMIN:   return "linear-gradient(135deg,#6366f1,#4f46e5)";
    case ROLES.TEACHER: return "linear-gradient(135deg,#8b5cf6,#7c3aed)";
    case ROLES.STUDENT: return "linear-gradient(135deg,#ec4899,#db2777)";
  }
}
...
export default function Sidebar({ user }: { user: AuthUser }) {
  ...
  // Derive this user's dashboard base path
  const basePath =
    user.role === ROLES.ADMIN   ? "/dashboard/admin"
    : user.role === ROLES.TEACHER ? "/dashboard/teacher"
    : "/dashboard/student";

  const navItems = ALL_NAV.filter((n) => n.roles.includes(user.role)).map((n) => ({
    ...n,
    href: n.href === "" ? basePath : n.href,
  }));
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "24px 20px 20px", borderBottom: "1px solid rgba(99,102,241,0.12)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontFamily: "var(--font-syne)", fontSize: 20, fontWeight: 800, background: "linear-gradient(135deg,#6366f1,#8b5cf6,#ec4899)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
            EduCore
          </span>
        </div>
      </div>
      ...
      <nav style={{ flex: 1, padding: "16px 12px", overflowY: "auto" }}>
        <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", color: "#6b6b80", textTransform: "uppercase", padding: "0 8px", marginBottom: 8 }}>
          Main Menu
        </p>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span style={{ color: active ? "#8b5cf6" : "#6b6b80", transition: "color 0.2s" }}>
                <item.icon />
              </span>
              {item.label}
            </Link>
      ...
      <div style={{ padding: "12px", borderTop: "1px solid rgba(99,102,241,0.12)" }}>
        ...
        <a
          ...
          style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 8, textDecoration: "none", color: "#6b6b80", fontSize: 13, marginBottom: 4, transition: "background 0.2s" }}
        ...
        <button
          onClick={logout}
          style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 8, background: "transparent", border: "none", color: "#6b6b80", fontSize: 13, cursor: "pointer", width: "100%", transition: "background 0.2s, color 0.2s", fontFamily: "var(--font-dm-sans)" }}
        ...
        {/* User card */}
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px", marginTop: 8, background: "rgba(99,102,241,0.06)", borderRadius: 12, border: "1px solid rgba(99,102,241,0.12)" }}>
          <div style={{ width: 36, height: 36, borderRadius: "50%", background: "linear-gradient(135deg,#6366f1,#ec4899)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", flexShrink: 0, fontFamily: "var(--font-syne)" }}>
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden"
        style={{ position: "fixed", top: 16, left: 16, zIndex: 200, width: 40, height: 40, borderRadius: 10, background: "rgba(99,102,241,0.15)", border: "1px solid rgba(99,102,241,0.3)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#e8e8f0" }}
      >
        <Icons.Menu />
      </button>
      ...
      {mobileOpen && (
        <div
          className="lg:hidden"
          onClick={() => setMobileOpen(false)}
          style={{ position: "fixed", inset: 0, zIndex: 149, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }}
        />
      )}
      ...
      <aside
        ...
        className={!mobileOpen ? "max-lg:translate-x-[-260px]" : ""}
      >
